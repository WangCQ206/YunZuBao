...
Page({
  ...
  getData: function () {
    ...
    var header = getApp().globalData.header;
  	wx.request({
      //url: 'http://47.110.253.98:8000/eqpList', 
      ...
      success: (res) => {
        ...
        if(dataList.length<=20&&dataList.length>0){
          eqp2=[];
        ...
        if(dataList.length>20){
          ...
          // for(var i=0;i<=dataList.length/2+1;i++){
          //   _eqp1[i]=dataList[i].split("+");
          //   console.log('_eqp1[]',_eqp1[i]);
          //   eqp1[i]=_eqp1[i][1];
          // }
          ...
          for(var i=20;i<dataList.length;i++,j++){
  ...
  changeTap: function (newId) {
    ...
    if(dataList.length==0){
      ...
      if(that.data.eqpStatus[that.data._index].status){
        wx.showModal({
          title: "当前是解机状态",
          showCancel: true,
          content: "是否要执行锁机操作",
...
// },
  onReady() {
    this.data.timer = setInterval(this.getData, 10000); //10秒刷新
  },
onUnload:function(){
